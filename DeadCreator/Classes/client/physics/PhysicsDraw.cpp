...
void PhysicsDraw::staticInit(PlayGameLayer* playGameLayer)
{
    instance.reset(new PhysicsDraw(playGameLayer));
}


PhysicsDraw::PhysicsDraw(PlayGameLayer* playGameLayer)
:_playGameLayer(playGameLayer)
{}
...
void PhysicsDraw::DrawPolygon(const b2Vec2* vertices, int32 vertexCount, const b2Color& color)
{
    ...
    for (size_t i = 0; i<vertexCount; i++)
    {
        ...
        auto a = _playGameLayer->worldToLocal(origin, toVec(vertices[i]));
        auto b = _playGameLayer->worldToLocal(origin, toVec(vertices[j]));
...
void PhysicsDraw::DrawCircle(const b2Vec2& center, float32 radius, const b2Color& color)
{
    auto origin = Vec2(ImGui::GetCursorScreenPos().x, ImGui::GetCursorScreenPos().y);
    auto drawList = ImGui::GetWindowDrawList();
    auto rad = radius * _playGameLayer->getGameLayer()->getGame()->getRenderingSysetm()->getZoomScale().x;
    auto pos = _playGameLayer->worldToLocal(origin, toVec(center));
...
void PhysicsDraw::DrawSegment(const b2Vec2& p1, const b2Vec2& p2, const b2Color& color)
{
    auto origin = Vec2(ImGui::GetCursorScreenPos().x, ImGui::GetCursorScreenPos().y);
    auto drawList = ImGui::GetWindowDrawList();
    auto a = _playGameLayer->worldToLocal(origin, toVec(p1));
    auto b = _playGameLayer->worldToLocal(origin, toVec(p2));
    drawList->AddLine(ImVec2(a.x, a.y), ImVec2(b.x, b.y), ImColor(ImVec4(1.0, 1.0, 0.0, 0.5)));
}
...
void PhysicsDraw::DrawPoint(const b2Vec2& p, float32 size, const b2Color& color)
{
    auto origin = Vec2(ImGui::GetCursorScreenPos().x, ImGui::GetCursorScreenPos().y);
    auto drawList = ImGui::GetWindowDrawList();
    Vec2 rectOrigin = _playGameLayer->worldToLocal(origin, toVec(p));
    Vec2 rectDest = _playGameLayer->worldToLocal(origin, Vec2(p.x + size, p.y + size));
