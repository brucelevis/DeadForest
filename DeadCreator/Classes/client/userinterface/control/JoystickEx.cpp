    ...
    _joystick->addTouchEventListener([&, this](Ref* ref, Widget::TouchEventType type){
        ...
        switch (type)
        {
            ...
            case Widget::TouchEventType::ENDED:
            {
                ...
                if ( _isEnableDoubleClick && _isDoubleClicked )
                {
...
void JoystickEx::disableJoystick()
{
    _joystick->setTouchEnabled(false);
    _joystick->setOpacity(80);
    if ( _pad ) _pad->setOpacity(30);
    
    _joystick->stopAllActions();
    _joystick->runAction(EaseElasticOut::create(MoveTo::create(0.2f, Vec2::ZERO)));
    _calculateDirection(Vec2::ZERO);
}


void JoystickEx::enableJoystick()
{
    _joystick->setTouchEnabled(true);
    _joystick->setSwallowTouches(false);
    _joystick->setOpacity(128);
    if ( _pad ) _pad->setOpacity(64);
}
