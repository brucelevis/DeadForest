...
Regulator::Regulator(double num_updates_per_second)
{
	...
	else if (isEqual(0.0, num_updates_per_second))
	{
		_update_period = 0.0;
	}
...
bool Regulator::isReady()
{
	...
	if (isEqual(0.0, _update_period)) return true;
	...
	{
		_next_update_time = (DWORD)(current_time + _update_period + cocos2d::random(-update_period_variator, update_period_variator));
