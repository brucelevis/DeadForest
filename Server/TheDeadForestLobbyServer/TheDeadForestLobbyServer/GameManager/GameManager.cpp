...
void GameManager::adjustPosition()
{
    cout << "[adjustPosition]GameManager" << endl;
    ...
    {
        Session* currSession = session.second;
        auto entity = static_cast <EntityHuman*>(getEntityFromID(_room->getPidToVid(currSession->getPid())));
        
        Vec2 worldPos = entity->getWorldPosition();
        Vec2 shootAt = entity->getHeading();
