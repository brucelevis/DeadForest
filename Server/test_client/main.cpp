...
#include "..\..\ASIO_TEST_SERVER\ASIO_TEST_SERVER\protocol.h"
...
const int FS_GAME_SERVER_PORT = MY_SERVER_PORT;
...
{
    switch (packet[1]) {
        case SC_GAME_OBJECT_MOVE: {
            sc_packet_object_move *P = reinterpret_cast<sc_packet_object_move *>(packet);
        ...
        case SC_END_GAME: {
            ...
            break;
        default: cout << "Unknown Packet Type\n";
            exit(-1);
    }
}
...
void Connect_Thread()
{
    ...
    clntAdr.sin_addr.s_addr = inet_addr(FS_GAME_SERVER_ADDRESS);
    clntAdr.sin_port = htons(MY_SERVER_PORT);
...
void DoAI(int id)
{
    switch (rand() % 4)
    {
        case 0: players[id].pos.z += 1.0; break;
        case 1: players[id].pos.z -= 1.0; break;
        case 2: players[id].pos.x += 1.0; break;
        case 3: players[id].pos.x -= 1.0; break;
    }
}
...
int main()
{
    ...
    do {
        ...
        for (auto i = 0; i < NUM_TEST; ++i)
        {
            ...
            my_packet.pos_x = players[i].pos.x;
            my_packet.pos_y = players[i].pos.y;
            my_packet.pos_z = players[i].pos.z;
            my_packet.rot_x = players[i].rot.x;
            my_packet.rot_y = players[i].rot.y;
            my_packet.rot_z = players[i].rot.z;
            my_packet.action_id = 2;
            Send_Packet(&my_packet, i);
        }
