...
namespace realtrick {
	...
	void Rect::merge(const Rect& rect)
	{
		...
		origin.x = std::min<float>(left1, left2);
		origin.y = std::min<float>(bottom1, bottom2);
		size.width = std::max<float>(right1, right2) - origin.x;
		size.height = std::max<float>(top1, top2) - origin.y;
	}
	...
	Rect Rect::unionWithRect(const Rect & rect) const
	{
		...
		float combinedLeftX = std::min<float>(thisLeftX, otherLeftX);
		float combinedRightX = std::max<float>(thisRightX, otherRightX);
		float combinedTopY = std::max<float>(thisTopY, otherTopY);
		float combinedBottomY = std::min<float>(thisBottomY, otherBottomY);
